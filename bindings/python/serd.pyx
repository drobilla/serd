...
cdef extern from "serd.h":
    ...
    SerdNode* serd_node_from_syntax(const char* str, SerdSyntax syntax, SerdEnv* env);

    char* serd_node_to_syntax(const SerdNode* node,
                              SerdSyntax      syntax,
                              const SerdEnv*  env);

    SerdNode* serd_new_token(SerdNodeType type, SerdStringView string);
    SerdNode* serd_new_string(SerdStringView string);
    SerdNode* serd_new_uri(SerdURIView uri);
    SerdNode* serd_new_file_uri(SerdStringView path, SerdStringView hostname);

    SerdNode* serd_new_literal(SerdStringView string,
                               SerdNodeFlags  flags,
                               SerdStringView meta);
    ...
    SerdNode* serd_new_integer(int64_t i, SerdStringView datatype);
    SerdNode* serd_new_base64(const void* buf, size_t size, SerdStringView datatype);
    ...
    SerdStatus serd_write_range(const SerdCursor*      range,
                                const SerdSink*        sink,
    ...
    SerdCaret* serd_caret_new(const SerdNode* name,
                                unsigned        line,
                                unsigned        col);

    SerdCaret* serd_caret_copy(const SerdCaret* caret);
    void        serd_caret_free(SerdCaret* caret);
...
class Status(enum.IntEnum):
    ...
    ERR_BAD_TEXT = 11  # Invalid text encoding
    ERR_BAD_WRITE = 12  # Error writing to file/stream
    ERR_NO_DATA = 13  # Unexpected end of input
    ERR_BAD_CALL = 14  # Invalid call
    ERR_BAD_URI = 15  # Invalid or unresolved URI
    ERR_BAD_INDEX = 16  # No optimal model index available
    ERR_INVALID = 17  # Invalid data
...
cdef class Node:
    ...
    @staticmethod
    def from_syntax(string: str, syntax: Syntax = Syntax.TURTLE, env: Env = None):
    ...
    def __init__(self, value):
        if isinstance(value, str):
            ...
            self._ptr = serd_new_integer(value, _empty_string)
    ...
    def to_syntax(self, syntax: Syntax = Syntax.TURTLE, env: Env = None) -> str:
        ...
        """
        cenv = env._ptr if env is not None else NULL
        cstr = serd_node_to_syntax(self._ptr, syntax, cenv)
...
def plain_literal(s: str, lang: str = None) -> Node:
    ...
    if lang is not None:
        ...
        return Node._manage(serd_new_literal(s_view, NodeFlag.HAS_LANGUAGE, lang_view))
    else:
        s_view = _string_view(s)
        return Node._manage(serd_new_string(s_view))
...
def typed_literal(s: str, datatype) -> Node:
    ...
    if type(datatype_node) == Node:
        ...
        return Node._manage(serd_new_literal(s_view, NodeFlag.HAS_DATATYPE, datatype_uri_view))
...
def blank(s: str) -> Node:
    s_view = _string_view(s)
    return Node._manage(serd_new_token(NodeType.BLANK, s_view))


def uri(s: str) -> Node:
    s_view = _string_view(s)
    return Node._manage(serd_new_token(NodeType.URI, s_view))
...
def decimal(
    ...
    return Node._manage(serd_new_decimal(d))
...
def integer(i: int, datatype: Node = None) -> Node:
    return Node._manage(
        serd_new_integer(i, serd_node_string_view(datatype._ptr) if datatype else _empty_string)
    )
...
def base64(const unsigned char[:] buf,
    ...
    return Node._manage(
        serd_new_base64(
            &buf[0],
            len(buf),
            serd_node_string_view(datatype._ptr) if datatype else _empty_string
        )
    )
...
def variable(s: str) -> Node:
    s_view = _string_view(s)
    return Node._manage(serd_new_token(NodeType.VARIABLE, s_view))
...
cdef class Cursor:
    ...
    def __init__(self, range: Cursor):
        ...
        self._ptr = serd_cursor_copy((<Cursor>range)._ptr)
...
cdef class Caret:
    ...
    @staticmethod
    cdef Caret _manage(SerdCaret* ptr):
        if ptr is NULL:
            return None
    ...
    cdef Caret _wrap(const SerdCaret* ptr):
        return Caret._manage(serd_caret_copy(ptr))

    def __init__(self, name, line: uint = 1, col: uint = 0):
        if type(name) == Node:
            self._name_node = <Node>name
            self._ptr = serd_caret_new((<Node>self._name_node)._ptr, line, col)
        elif type(name) == str:
    ...
    def name(self) -> Node:
        ...
        return Node._wrap(serd_caret_name(self._ptr))
