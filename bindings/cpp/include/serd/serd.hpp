...
private:
  ...
  static std::string format(const char* fmt, va_list args) noexcept
  {
    ...
    if (vsnprintf(result.data(), n_bytes + 1U, fmt, args) != n_bytes) {
      return "";
    }
#else
    std::vector<char> str(static_cast<size_t>(n_bytes) + 1U, '\0');
    if (vsnprintf(str.data(), n_bytes + 1u, fmt, args) != n_bytes) {
      return "";
    }
...
public:
  explicit Env(World& world)
    : EnvWrapper{serd_env_new(world.cobj(), zix_empty_string())}
  {}
