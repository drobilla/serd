...
test_comparison(void)
{
  ...
  const SerdNode* const a =
    serd_nodes_get(nodes, serd_as_uri_string("http://example.org/a"));

  const SerdNode* const b =
    serd_nodes_get(nodes, serd_as_uri_string("http://example.org/b"));

  const SerdNode* const c =
    serd_nodes_get(nodes, serd_as_uri_string("http://example.org/c"));
