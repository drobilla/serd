...
def test_suite(
    ...
    model, instances = serd_test_util.load_rdf(command_prefix + ["-B", base_uri], manifest_path)
    ...
    def run_tests(test_class, tests, expected_return, results):
        ...
        for test in sorted(tests):
            ...
            out_filename = os.path.join(out_test_dir, _add_extension(test_name, ".sort"))
            ...
            if report_filename:
                with open(report_filename, "a") as report:
                    report.write(serd_test_util.earl_assertion(test, passed, asserter))
...
def main():
    ...
    command_prefix = (
        shlex.split(args.wrapper) + [args.tool] + args.serdi_option
    )
