...
test_caret(void)
{
  ...
  const SerdNode* const node  = serd_nodes_get(nodes, serd_as_string("node"));
  ...
  const SerdNode* const other_node =
    serd_nodes_get(nodes, serd_as_string("other"));
...
test_failed_alloc(void)
{
  ...
  assert(
    !serd_node_construct(sizeof(node_buf), node_buf, serd_as_string("node"))
       .status);
