...
static const size_t min_stack_size = 244U;
...
test_turtle_overflow(void)
{
  static const char* const test_strings[] = {
    ...
    "<http://example.org/s> <http://example.org/p> \"\"\"not\"\\nend\"\"\" .",
    "<http://example.org/s> <http://example.org/p> _:blank .",
    ...
    "PREFIX abomination: <http://example.org/spurqle>\n"
...
test_trig_overflow(void)
{
  ...
  for (const char* const* t = test_strings; *t; ++t) {
    test_all_sizes(world, *t, SERD_TRIG);
  }
