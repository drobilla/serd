...
uri(SerdWorld* world, const unsigned num)
{
  ...
  return serd_nodes_get(serd_world_nodes(world), serd_as_uri_string(str));
}
         ...
         const SerdNode* graph)
{
  ...
  const SerdNode* hello = serd_nodes_get(nodes, serd_as_string("hello"));

  const SerdNode* hello_gb = serd_nodes_get(
    nodes, serd_as_plain_literal(serd_string("hello"), serd_string("en-gb")));

  const SerdNode* hello_us = serd_nodes_get(
    nodes, serd_as_plain_literal(serd_string("hello"), serd_string("en-us")));

  const SerdNode* hello_t4 =
    serd_nodes_get(nodes,
                   serd_as_typed_literal(serd_string("hello"),
  ...
  const SerdNode* const bonjour = serd_nodes_get(
    nodes, serd_as_plain_literal(serd_string("bonjour"), serd_string("fr")));

  const SerdNode* const salut = serd_nodes_get(
    nodes, serd_as_plain_literal(serd_string("salut"), serd_string("fr")));
  ...
  const SerdNode* ablank =
    serd_nodes_get(nodes, serd_as_blank(serd_string("ablank")));
          ...
          const unsigned  n_quads)
{
  ...
  const SerdNode* plain_hello = serd_nodes_get(nodes, serd_as_string_view(s));

  const SerdNode* type4_hello = serd_nodes_get(
    nodes, serd_as_typed_literal(s, serd_node_string_view(uri(world, 4))));

  const SerdNode* type5_hello = serd_nodes_get(
    nodes, serd_as_typed_literal(s, serd_node_string_view(uri(world, 5))));

  const SerdNode* gb_hello =
    serd_nodes_get(nodes, serd_as_plain_literal(s, serd_string("en-gb")));

  const SerdNode* us_hello =
    serd_nodes_get(nodes, serd_as_plain_literal(s, serd_string("en-us")));
...
test_inserter(SerdWorld* world, const unsigned n_quads)
{
  ...
  const SerdNode* const s =
    serd_nodes_get(nodes, serd_as_uri_string("http://example.org/s"));

  const SerdNode* const p =
    serd_nodes_get(nodes, serd_as_uri_string("http://example.org/p"));

  const SerdNode* const rel = serd_nodes_get(nodes, serd_as_uri_string("rel"));
...
test_add_erase(SerdWorld* world, const unsigned n_quads)
{
  ...
  // Add (s p "hi")
  const SerdNode* hi = serd_nodes_get(nodes, serd_as_string("hi"));
...
test_add_bad_statement(SerdWorld* world, const unsigned n_quads)
{
  ...
  const SerdNode* s = serd_nodes_get(nodes, serd_as_uri_string("urn:s"));
  const SerdNode* p = serd_nodes_get(nodes, serd_as_uri_string("urn:p"));
  const SerdNode* o = serd_nodes_get(nodes, serd_as_uri_string("urn:o"));

  const SerdNode* f =
    serd_nodes_get(nodes, serd_as_uri_string("file:///tmp/file.ttl"));
...
test_add_with_caret(SerdWorld* world, const unsigned n_quads)
{
  ...
  const SerdNode*  lit   = serd_nodes_get(nodes, serd_as_string("string"));
  const SerdNode*  uri   = serd_nodes_get(nodes, serd_as_uri_string("urn:uri"));

  const SerdNode* blank =
    serd_nodes_get(nodes, serd_as_blank(serd_string("b1")));
...
test_write_flat_range(SerdWorld* world, const unsigned n_quads)
{
  ...
  const SerdNode* s  = serd_nodes_get(nodes, serd_as_uri_string("urn:s"));
  const SerdNode* p  = serd_nodes_get(nodes, serd_as_uri_string("urn:p"));
  const SerdNode* b1 = serd_nodes_get(nodes, serd_as_blank(serd_string("b1")));
  const SerdNode* b2 = serd_nodes_get(nodes, serd_as_blank(serd_string("b2")));
  const SerdNode* o  = serd_nodes_get(nodes, serd_as_uri_string("urn:o"));
...
test_write_bad_list(SerdWorld* world, const unsigned n_quads)
{
  ...
  const SerdNode* list1 =
    serd_nodes_get(nodes, serd_as_blank(serd_string("l1")));

  const SerdNode* list2 =
    serd_nodes_get(nodes, serd_as_blank(serd_string("l2")));

  const SerdNode* nofirst =
    serd_nodes_get(nodes, serd_as_blank(serd_string("nof")));

  const SerdNode* norest =
    serd_nodes_get(nodes, serd_as_blank(serd_string("nor")));

  const SerdNode* pfirst = serd_nodes_get(nodes, serd_as_uri_string(RDF_FIRST));
  const SerdNode* prest  = serd_nodes_get(nodes, serd_as_uri_string(RDF_REST));

  const SerdNode* val1 = serd_nodes_get(nodes, serd_as_string("a"));
  const SerdNode* val2 = serd_nodes_get(nodes, serd_as_string("b"));
...
test_write_error_in_list_subject(SerdWorld* world, const unsigned n_quads)
{
  ...
  const SerdNode* p   = serd_nodes_get(nodes, serd_as_uri_string("urn:p"));
  const SerdNode* o   = serd_nodes_get(nodes, serd_as_uri_string("urn:o"));
  const SerdNode* l1  = serd_nodes_get(nodes, serd_as_blank(serd_string("l1")));
  const SerdNode* one = serd_nodes_get(nodes, serd_as_integer(1));
  const SerdNode* l2  = serd_nodes_get(nodes, serd_as_blank(serd_string("l2")));
  const SerdNode* two = serd_nodes_get(nodes, serd_as_integer(2));

  const SerdNode* rdf_first =
    serd_nodes_get(nodes, serd_as_uri_string(RDF_FIRST));

  const SerdNode* rdf_rest =
    serd_nodes_get(nodes, serd_as_uri_string(RDF_REST));

  const SerdNode* rdf_nil = serd_nodes_get(nodes, serd_as_uri_string(RDF_NIL));
