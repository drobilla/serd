...
static inline bool
token_view_matches(const SerdTokenView a, const SerdNode* const ZIX_NULLABLE b)
{
  return !b || serd_node_equals_token_view(b, a);
}

static inline bool
object_view_matches(const SerdObjectView               a,
                    const SerdNode* const ZIX_NULLABLE b)
{
  return !b || serd_node_equals_object_view(b, a);
}
...
statement_view_matches(const SerdStatementView statement,
                       ...
                       const SerdNode* const   graph)
{
  return (token_view_matches(statement.subject, subject) &&
          token_view_matches(statement.predicate, predicate) &&
          object_view_matches(statement.object, object) &&
          token_view_matches(statement.graph, graph));
}
...
{
  static const SerdTokenView  no_tok = {{"", 0U}, SERD_LITERAL};
  static const SerdObjectView no_obj = {{"", 0U}, SERD_LITERAL, 0U, no_tok};
  ...
  SerdStatementView prev   = {no_tok, no_tok, no_obj, no_tok};
...
#define NUM_PATTERNS 18
  ...
  for (unsigned i = 0; i < NUM_PATTERNS; ++i) {
    ...
    for (; !serd_cursor_is_end(range); serd_cursor_advance(range)) {
      ...
      /* assert(first.object); */
      assert(token_view_matches(first.subject, pat[0]));
      assert(token_view_matches(first.predicate, pat[1]));
      assert(object_view_matches(first.object, pat[2]));
      assert(token_view_matches(first.graph, pat[3]));
    }
...
#undef NUM_PATTERNS
  ...
  for (; !serd_cursor_is_end(range); serd_cursor_advance(range)) {
    ...
    /* assert(statement.predicate); */
    /* assert(statement.object); */
    assert(token_view_matches(statement.subject, pat[0]));
    assert(token_view_matches(statement.predicate, pat[1]));
    assert(object_view_matches(statement.object, pat[2]));
    assert(token_view_matches(statement.graph, pat[3]));
  }
  serd_cursor_free(NULL, range);
  ...
  // Test nested queries
  SerdTokenView last_subject = {{"", 0U}, SERD_LITERAL};
  range = serd_model_find(NULL, model, NULL, NULL, NULL, NULL);
  for (; !serd_cursor_is_end(range); serd_cursor_advance(range)) {
    const SerdStatementView statement = serd_cursor_get(range);
    const SerdTokenView     subject   = statement.subject;
    if (token_view_equals(subject, last_subject)) {
      continue;
    }

    // FIXME: blech
    Quad subpat = {serd_nodes_get(nodes, serd_a_token_view(subject)), 0, 0};
    ...
    assert(token_view_equals(substatement.subject, subject));
    ...
    // FIXME: blech
    uint64_t count = serd_model_count(
      model, serd_nodes_get(nodes, serd_a_token_view(subject)), 0, 0, 0);
...
{
  static const SerdTokenView no_document = {{"", 0U}, SERD_LITERAL};
  static const SerdCaretView no_caret    = {no_document, 0U, 0U};
  ...
  assert(serd_sink_write_event(
           ...
           serd_statement_event(0U,
                                serd_statement_view_nodes(s, p, rel, NULL),
                                no_caret)) == SERD_BAD_DATA);
...
test_add_bad_statement(SerdWorld* const world, const unsigned n_quads)
{
  ...
  const SerdCaretView origin = {serd_node_token_view(f), 16, 18};

  SerdModel* const model = serd_model_new(world, SERD_ORDER_SPO, 0U);

  assert(!serd_model_add_from(model, s, p, o, NULL, origin));

  SerdCursor* const       begin     = serd_model_begin(NULL, model);
  const SerdStatementView statement = serd_cursor_get(begin);
  const SerdCaretView     caret     = serd_cursor_get_caret(begin);

  assert(serd_node_equals_token_view(s, statement.subject));
  assert(serd_node_equals_token_view(p, statement.predicate));
  assert(serd_node_equals_object_view(o, statement.object));
  // FIXME
  // assert(!statement.graph);

  assert(serd_node_equals_token_view(f, caret.document));
  assert(caret.line == 16);
  assert(caret.column == 18);
...
test_write_flat_range(SerdWorld* const world, const unsigned n_quads)
{
  ...
  while (!serd_cursor_is_end(all)) {
    serd_sink_write_event(
      serd_writer_sink(writer),
      serd_statement_event(0U, serd_cursor_get(all), no_caret));
