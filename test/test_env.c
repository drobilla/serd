...
test_base_uri(void)
{
  ...
  assert(serd_node_equals(serd_env_base_node(env), eg));
...
test_expand_curie(void)
{
  ...
  assert(!serd_env_expand(env, prefixed, &prefix, &suffix));
  assert(prefix.data);
  assert(!strcmp(prefix.data, "http://example.org/"));
  assert(suffix.data);
