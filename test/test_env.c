...
test_base_uri(void)
{
  ...
  SerdEnv* const        env   = serd_env_new(world, serd_empty_string());
  const SerdNode* const eg = serd_nodes_get(nodes, serd_as_uri_string(NS_EG));
...
test_expand_untyped_literal(void)
{
  SerdWorld* const      world   = serd_world_new(NULL);
  SerdNodes* const      nodes   = serd_world_nodes(world);
  const SerdNode* const untyped = serd_nodes_get(nodes, serd_as_string("data"));
...
test_expand_bad_uri_datatype(void)
{
  ...
  const SerdNode* const typed =
    serd_nodes_get(nodes, serd_as_typed_literal(serd_string("data"), type));
...
test_expand_uri(void)
{
  ...
  const SerdNode* const empty = serd_nodes_get(nodes, serd_as_uri_string(""));
...
test_expand_bad_uri(void)
{
  ...
  const SerdNode* const bad_uri =
    serd_nodes_get(nodes, serd_as_uri_string("rel"));
