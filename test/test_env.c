...
test_env(void)
{
  const ZixStringView prefix = zix_string("eg.2");
  const ZixStringView eg     = zix_string("http://example.org/");

  SerdNode* hello = serd_new_string(zix_string("hello\""));
  SerdNode* rel   = serd_new_uri(zix_string("rel"));
  SerdNode* foo_u = serd_new_uri(zix_string("http://example.org/foo"));
  SerdNode* foo_c = serd_new_curie(zix_string("eg.2:foo"));
  ...
  serd_env_set_prefix(
    env, zix_string("eg.2"), zix_string("http://example.org/"));
  ...
  SerdNode* unqualifiable = serd_new_uri(zix_string("http://drobilla.net/"));
  assert(!serd_env_qualify(env, unqualifiable));
  serd_node_free(unqualifiable);

  assert(!serd_env_expand_node(env, rel));
  serd_env_set_base_uri(env, zix_string("http://example.org/base/"));
