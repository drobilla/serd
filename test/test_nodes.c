...
test_string(void)
{
  ...
  const SerdNode* const long_node =
    serd_nodes_literal(nodes, string, SERD_IS_LONG, serd_empty_string());
...
test_plain_literal(void)
{
  ...
  const SerdNode* const long_version = serd_nodes_literal(
    nodes, string, SERD_HAS_LANGUAGE | SERD_IS_LONG, language);
...
test_base64(void)
{
  ...
  assert(!strcmp(serd_node_string(custom_datatype), user_datatype.data));
...
test_file_uri(void)
{
  ...
  const SerdNode* const local_uri = serd_nodes_uri(nodes, local_string);
  const SerdNode* const local_file_uri =
    serd_nodes_file_uri(nodes, local_path, serd_empty_string());
  ...
  const SerdNode* const remote_uri = serd_nodes_uri(nodes, remote_string);
  const SerdNode* const remote_file_uri =
    serd_nodes_file_uri(nodes, local_path, remote_host);
