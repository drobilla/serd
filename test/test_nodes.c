...
test_intern_failed_alloc(void)
{
  ...
  SerdNode* const node = serd_node_new(&allocator.base, serd_as_string("node"));
...
test_intern(void)
{
  ...
  SerdNode*  node  = serd_node_new(NULL, serd_as_string("node"));
...
test_string(void)
{
  ...
  const SerdNode* const node =
    serd_nodes_get(nodes, serd_as_string_view(string));
...
test_invalid_literal(void)
{
  ...
  assert(!serd_nodes_get(nodes,
                         serd_as_literal(serd_string("double meta"),
                                         SERD_HAS_LANGUAGE | SERD_HAS_DATATYPE,
                                         serd_string("What am I?"))));

  assert(!serd_nodes_get(nodes,
                         serd_as_literal(serd_string("empty language"),
                                         SERD_HAS_LANGUAGE,
                                         serd_empty_string())));

  assert(!serd_nodes_get(nodes,
                         serd_as_literal(serd_string("empty datatype"),
                                         SERD_HAS_DATATYPE,
                                         serd_empty_string())));
...
test_plain_literal(void)
{
  ...
  SerdNodes* const      nodes = serd_nodes_new(allocator);
  const SerdNode* const node =
    serd_nodes_get(nodes, serd_as_plain_literal(string, language));
  ...
  const SerdNode* const other =
    serd_nodes_get(nodes, serd_as_plain_literal(string, serd_string("de")));
...
test_typed_literal(void)
{
  ...
  SerdNodes* const      nodes = serd_nodes_new(allocator);
  const SerdNode* const node =
    serd_nodes_get(nodes, serd_as_typed_literal(string, datatype));
...
test_boolean(void)
{
  ...
  const SerdNode* const false1 =
    serd_nodes_get(nodes, serd_as_primitive(serd_bool(false)));

  const SerdNode* const false2 =
    serd_nodes_get(nodes, serd_as_primitive(serd_bool(false)));

  const SerdNode* const true1 =
    serd_nodes_get(nodes, serd_as_primitive(serd_bool(true)));
...
test_decimal(void)
{
  ...
  const SerdNode* const a = serd_nodes_get(nodes, serd_as_decimal(-12.3456789));
  const SerdNode* const b = serd_nodes_get(nodes, serd_as_decimal(-12.3456789));
...
test_double(void)
{
  ...
  const SerdNode* const a =
    serd_nodes_get(nodes, serd_as_primitive(serd_double(-1.2E3)));
...
test_float(void)
{
  ...
  const SerdNode* const a =
    serd_nodes_get(nodes, serd_as_primitive(serd_float(-1.2E3f)));
...
test_integer(void)
{
  ...
  const SerdNode* const a = serd_nodes_get(nodes, serd_as_integer(-1234567890));
  const SerdNode* const b = serd_nodes_get(nodes, serd_as_integer(-1234567890));
...
test_base64(void)
{
  ...
  const SerdNode* const a =
    serd_nodes_get(nodes, serd_as_base64(sizeof(data), &data));
...
test_uri(void)
{
  ...
  const SerdNode* const node = serd_nodes_get(nodes, serd_as_uri(string));
...
test_parsed_uri(void)
{
  ...
  const SerdNode* const node  = serd_nodes_get(nodes, serd_as_parsed_uri(uri));
  ...
  const SerdNode* const alias = serd_nodes_get(nodes, serd_as_parsed_uri(uri));

  assert(alias == node);

  const SerdNode* const other = serd_nodes_get(
    nodes, serd_as_parsed_uri(serd_parse_uri("http://example.org/x")));
...
test_blank(void)
{
  ...
  SerdNodes* const      nodes = serd_nodes_new(allocator);
  const SerdNode* const node  = serd_nodes_get(nodes, serd_as_blank(string));
...
test_deref(void)
{
  ...
  const SerdNode* original = serd_nodes_get(nodes, serd_as_string("node"));
  const SerdNode* another  = serd_nodes_get(nodes, serd_as_string("node"));
  ...
  const SerdNode* other = serd_nodes_get(nodes, serd_as_string("XXXX"));
  assert(!strcmp(serd_node_string(other), "XXXX"));

  // Intern a new equivalent node to the original and check that it's really new
  const SerdNode* imposter = serd_nodes_get(nodes, serd_as_string("node"));
  ...
  SerdNode* unmanaged = serd_node_new(NULL, serd_as_string("unmanaged"));
