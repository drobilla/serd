...
test_writer(const char* const path)
{
  ...
  const SerdNode* lit = serd_nodes_get(nodes, serd_as_string("hello"));
  ...
  const SerdNode* p =
    serd_nodes_get(nodes, serd_as_uri_string("http://example.org/pred"));

  const SerdNode* bad =
    serd_nodes_get(nodes, serd_as_string_view(bad_buf_view));
  ...
  const SerdNode* const o        = serd_nodes_get(nodes, serd_as_string("o"));

  const SerdNode* const t =
    serd_nodes_get(nodes, serd_as_typed_literal(serd_string("t"), urn_Type));

  const SerdNode* const l =
    serd_nodes_get(nodes, serd_as_plain_literal(serd_string("l"), en));
  ...
  const SerdNode* bad_lit = serd_nodes_get(nodes, serd_as_string(bad_lit_str));
  const SerdNode* bad_uri = serd_nodes_get(nodes, serd_as_string(bad_uri_str));

  assert(!serd_sink_write(iface, 0, s, p, bad_lit, 0));
  assert(!serd_sink_write(iface, 0, s, p, bad_uri, 0));

  // Write 1 valid statement
  const SerdNode* const hello = serd_nodes_get(nodes, serd_as_string("hello"));
  ...
  const SerdNode* const base =
    serd_nodes_get(nodes, serd_as_uri_string("http://example.org/base"));
