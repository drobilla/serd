...
test_common(SerdWorld* const world, const SerdSyntax syntax)
{
  ...
  assert(check(
    world, syntax, serd_nodes_get(nodes, serd_as_string("node")), "\"node\""));

  assert(check(
    world,
    syntax,
    serd_nodes_get(
      nodes, serd_as_plain_literal(serd_string("hallo"), serd_string("de"))),
    "\"hallo\"@de"));

  assert(check(
    world,
    syntax,
    serd_nodes_get(nodes, serd_as_typed_literal(serd_string("X"), datatype)),
    "\"X\"^^<http://example.org/Datatype>"));

  assert(check(world,
               syntax,
               serd_nodes_get(nodes, serd_as_blank(serd_string("blank"))),
               "_:blank"));

  assert(check(world,
               syntax,
               serd_nodes_get(nodes, serd_as_blank(serd_string("b0"))),
               "_:b0"));

  assert(check(world,
               syntax,
               serd_nodes_get(nodes, serd_as_blank(serd_string("named1"))),
               "_:named1"));

  assert(check(world,
               syntax,
               serd_nodes_get(nodes, serd_as_uri_string("http://example.org/")),
               "<http://example.org/>"));

  assert(check(world,
               syntax,
               serd_nodes_get(nodes, serd_as_primitive(serd_double(1.25))),
               "\"1.25E0\"^^<http://www.w3.org/2001/XMLSchema#double>"));

  assert(check(world,
               syntax,
               serd_nodes_get(nodes, serd_as_primitive(serd_float(1.25f))),
               "\"1.25E0\"^^<http://www.w3.org/2001/XMLSchema#float>"));

  assert(check(world,
               syntax,
               serd_nodes_get(nodes, serd_as_hex(sizeof(data), data)),
...
test_ntriples(void)
{
...

  {
    ...
    const SerdNode* const rel =
      serd_nodes_get(nodes, serd_as_uri_string("rel/uri"));
  ...
  assert(check(world,
               SERD_NTRIPLES,
               serd_nodes_get(nodes, serd_as_decimal(1.25)),
               "\"1.25\"^^<http://www.w3.org/2001/XMLSchema#decimal>"));

  assert(check(world,
               SERD_NTRIPLES,
               serd_nodes_get(nodes, serd_as_integer(1234)),
...
test_turtle(void)
{
  ...
  assert(check(
    world, SERD_TURTLE, serd_nodes_get(nodes, serd_as_decimal(1.25)), "1.25"));

  assert(check(
    world, SERD_TURTLE, serd_nodes_get(nodes, serd_as_integer(1234)), "1234"));
