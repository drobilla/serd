...
test_fread_wrapper(void* const buf, const size_t len, void* const stream)
{
...
prepare_test_read(void* const buf, const size_t len, void* const stream)
{
  assert(len == 1U);
...
check_cursor(void* handle, const SerdEvent* event)
{
  ...
  if (event->type == SERD_STATEMENT) {
    ...
    assert(
      !strcmp(serd_node_string((const SerdNode*)caret.document), "string"));
    assert(caret.line == 1);
