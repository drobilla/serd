...
test_list_subject_failed_alloc(void)
{
  ...
  assert(!add(model, nodes, ids.l1, ids.rdf_first, ids.a, 0U));
  assert(!add(model, nodes, ids.l1, ids.rdf_rest, ids.rdf_nil, 0U));
  assert(!add(model, nodes, ids.l1, ids.p, ids.o, 0U));
...
test_bad_list(void)
{
  ...
  assert(!add(model, nodes, ids.s, ids.p, ids.l1, 0U));
  assert(!add(model, nodes, ids.l1, ids.rdf_first, ids.a, 0U));
  assert(!add(model, nodes, ids.l1, ids.rdf_rest, nofirst, 0U));

  // List where second node has no rdf:rest
  assert(!add(model, nodes, ids.s, ids.p, ids.l2, 0U));
  assert(!add(model, nodes, ids.l2, ids.rdf_first, ids.a, 0U));
  assert(!add(model, nodes, ids.l2, ids.rdf_rest, norest, 0U));
  assert(!add(model, nodes, norest, ids.rdf_first, ids.b, 0U));
...
test_infinite_list(void)
{
  ...
  assert(!add(model, nodes, ids.s, ids.p, ids.l1, 0U));
  assert(!add(model, nodes, ids.l1, ids.rdf_first, ids.a, 0U));
  assert(!add(model, nodes, ids.l1, ids.rdf_rest, ids.l2, 0U));
  assert(!add(model, nodes, ids.l2, ids.rdf_first, ids.b, 0U));
  assert(!add(model, nodes, ids.l2, ids.rdf_rest, ids.l1, 0U));
...
test_error_in_list_subject(void)
{
  ...
  assert(!add(model, nodes, ids.l1, ids.rdf_first, ids.one, 0U));
  assert(!add(model, nodes, ids.l1, ids.rdf_rest, ids.l2, 0U));
  assert(!add(model, nodes, ids.l2, ids.rdf_first, ids.two, 0U));
  assert(!add(model, nodes, ids.l2, ids.rdf_rest, ids.rdf_nil, 0U));
  assert(!add(model, nodes, ids.l1, ids.p, ids.o, 0U));
...
test_cursor(void)
{
  ...
  assert(!serd_model_add_from(model,
                              serd_a_uri(zix_string("http://example.org/s")),
                              serd_a_uri(zix_string("http://example.org/p")),
                              serd_a_uri(zix_string("http://example.org/o")),
                              serd_a_null(),
                              caret));
