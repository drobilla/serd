...
test_callbacks(void)
{
  ...
  const SerdNode* base = serd_nodes_get(nodes, serd_as_uri_string(NS_EG));
  const SerdNode* name = serd_nodes_get(nodes, serd_as_string("eg"));
  const SerdNode* uri  = serd_nodes_get(nodes, serd_as_uri_string(NS_EG "uri"));
