...
/// Return true iff `model` has an index for a specific statement order
SERD_PURE_API bool
...
/**
   Add a statement to a model with a caret.

   This function fails if there are any active iterators on `model`.
   If statement is null, then SERD_FAILURE is returned.
*/
SERD_API SerdStatus
serd_model_insert_from(SerdModel* ZIX_NONNULL model,
...
/**
   ...
   @param model The model to insert statements into.

   @param cursor A cursor over the range of statements to add. This can refer
   to any model, if it refers to `model`, this call does nothing (since the
...
/**
   ...
   @param model The model that `cursor` points to.
