...
on_pattern_event(void* const handle, const SerdEvent* const event)
{
  ...
  if (event->type == SERD_STATEMENT) {
    ...
    pat->s = serd_node_new(
      allocator,
      serd_a_token(statement.subject.type, statement.subject.string));
    pat->p = serd_node_new(
      allocator,
      serd_a_token(statement.predicate.type, statement.predicate.string));
    pat->o = serd_node_new(
      allocator, serd_a_token(statement.object.type, statement.object.string));
    pat->g = serd_field_supports(SERD_GRAPH, statement.graph.type)
               ? serd_node_new(
                   allocator,
                   serd_a_token(statement.graph.type, statement.graph.string))
               : NULL;
  }
