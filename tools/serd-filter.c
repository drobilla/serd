...
on_pattern_event(void* const handle, const SerdEvent* const event)
{
  ...
  if (event->type == SERD_EVENT_STATEMENT) {
    ...
    pat->g_string = zix_string_view_copy(allocator, statement.graph.string);
    pat->s_string = zix_string_view_copy(allocator, statement.subject.string);
    pat->p_string = zix_string_view_copy(allocator, statement.predicate.string);
    pat->o_string = zix_string_view_copy(allocator, statement.object.string);
    pat->meta_string =
      zix_string_view_copy(allocator, statement.object.meta.string);

    pat->g = serd_token_view(statement.graph.type, zix_string(pat->g_string));
    pat->s = serd_token_view(statement.subject.type, zix_string(pat->s_string));
    pat->p =
      serd_token_view(statement.predicate.type, zix_string(pat->p_string));
    pat->o = serd_object_view(statement.object.type,
                              zix_string(pat->o_string),
                              statement.object.flags,
                              serd_token_view(statement.object.meta.type,
                                              zix_string(pat->meta_string)));
  }
              ...
              const bool             inclusive)
{
  ...
  PatternEventContext ctx = {allocator,
                             {NULL,
                              NULL,
                              NULL,
                              NULL,
  ...
  const SerdSink in_sink = {&ctx, on_pattern_event};

  SerdReader* const reader =
    serd_reader_new(world, SERD_NQUADS, SERD_READ_VARIABLES, &in_sink);

  SerdStatus st = serd_reader_start(reader, in, zix_string("pattern"), 1);
  if (!st) {
  ...
  SerdHandler* filter = NULL;
  if (!st) {
    filter = serd_filter_new(world,
                             env,
                             sink,
                             ctx.pattern.s,
                             ctx.pattern.p,
  ...
  zix_free(allocator, ctx.pattern.g_string);
  zix_free(allocator, ctx.pattern.s_string);
  zix_free(allocator, ctx.pattern.p_string);
  zix_free(allocator, ctx.pattern.o_string);
  zix_free(allocator, ctx.pattern.meta_string);
  return filter;
}
...
run(Options opts)
{
  ...
  // Parse the pattern into an event handler
  SerdHandler* const filter =
    parse_pattern(app.world, app.env, target, &pattern, !opts.invert);
  if (!filter) {
  ...
  //                              \-> filter -> writer
  SerdHandler* const output =
    serd_tee_new(NULL, serd_env_sink(app.env), serd_handler_sink(filter));

  // Read all the inputs, which drives the writer to emit the output
  if (!(st = serd_read_inputs(&app,
                              opts.common,
                              opts.n_inputs,
                              opts.inputs,
                              serd_handler_sink(output)))) {
...
print_usage(const char* const name, const bool error)
{
  static const char* const description =
    ...
    "            decoded/generated/global/lax/prefixed/relative/variables\n"
    "  -O SYNTAX Output syntax empty/nquads/ntriples/trig/turtle, or option\n"
    "            ascii/baseless/encoded/expanded/prefixless/terse/verbatim\n"
    "  -V        Display version information and exit\n"
    "  -f FILE   Read pattern from FILE instead\n"
...
main(int argc, char** argv)
{
  ...
  if (opts.n_inputs == 1) {
    opts.common.input.flags |= (unsigned)SERD_READ_GLOBAL;
  }
