...
print_usage(const char* const name, const bool error)
{
  static const char* const description =
    ...
    "  -B BASE_URI  Base URI\n"
    "  -I SYNTAX    Input syntax (nquads/ntriples/trig/turtle),\n"
    "               or flag (generated/global/lax/variables)\n"
    "  -O SYNTAX    Output syntax (empty/nquads/ntriples/trig/turtle),\n"
          ...
          const size_t          block_size)
{
  ...
  SerdStatus st =
    serd_reader_start(reader, &in, zix_string(filename), block_size);
...
main(int argc, char** argv)
{
  ...
  if (base_arg) { // Base URI given on command line
    if (serd_uri_string_has_scheme(base_arg)) {
      ...
      serd_env_set_base_path(env, zix_string(base_arg));
    }
  }
  ...
  for (int i = 0; !st && i < n_inputs; ++i) {
    if (!base_arg && !!strcmp(inputs[i], "-")) {
      serd_env_set_base_path(env, zix_string(inputs[i]));
    }
