...
run(const Options opts)
{
  ...
  const SerdModelFlags flags =
    (SerdModelFlags)(with_graphs * SERD_MODEL_GRAPHS);

  SerdNodes* const nodes = serd_nodes_new(NULL);
  SerdModel* const model =
    serd_model_new(app.world, nodes, default_order, flags);
  ...
  SerdHandler* const inserter =
    serd_inserter_new(model, app.env, serd_no_token());
  SerdHandler* const loader =
    serd_tee_new(NULL, serd_env_sink(app.env), serd_handler_sink(inserter));
  if (st || (st = serd_read_inputs(&app,
                                   opts.common,
                                   opts.n_inputs,
                                   opts.inputs,
                                   serd_handler_sink(loader)))) {
    serd_handler_free(loader);
      ...
      st = serd_sink_event(
        ...
        serd_cite_event(
          serd_statement_event(
            0U, serd_strings_statement(strings, serd_cursor_tuple(cursor))),
          serd_strings_caret(strings, serd_cursor_caret(cursor))));
...
print_usage(const char* const name, const bool error)
{
  static const char* const description =
    ...
    "option\n"
    "                ascii/contextual/encoded/expanded/terse/verbatim\n"
    "  -V            Display version information and exit\n"
    "  -b BYTES      I/O block size\n"
    "  -c COLLATION  Ordering, an optional \"G\" then \"SPO\" in any order\n"
...
main(const int argc, char* const* const argv)
{
  ...
  if (opts.common.output.flags & (unsigned)SERD_WRITE_LONGHAND) {
    opts.flags |= (unsigned)SERD_DESCRIBE_SORTED_TYPE;
  }
