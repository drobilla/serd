...
A new model can be created with :func:`serd_model_new`, which takes a node set, default statement order, and a bitwise OR of flags:
...
A :struct:`SerdNodes` can be used by several models, allowing the memory to be shared, and IDs to be comarable between models.

Flags can be used to control what information is stored for each statement.
With :enumerator:`SERD_MODEL_CARETS`,
the original document position is retained and available via  :func:`serd_cursor_caret`,
and with :enumerator:`SERD_MODEL_GRAPHS`,
statements have an additional graph field.

The default order sets the order of the ever-present primary index,
which dictates the order of statements when iterating over the model.
Additional indexes can be enabled with :func:`serd_model_add_index`.
For example, to be able to quickly search by predicate,
an index where the predicate is most significant,
like :enumerator:`SERD_ORDER_PSO`,
can be added:

.. literalinclude:: overview_code.c
   :start-after: begin model-add-index
   :end-before: end model-add-index
   :dedent: 2
...
Statements can be added to a model with :func:`serd_model_insert`:
...
An entire range of statements can be inserted at once with :func:`serd_model_insert_range`.
For example, all statements in one model could be copied into another like so:

.. literalinclude:: overview_code.c
   :start-after: begin model-insert-range
   :end-before: end model-insert-range
   :dedent: 2

Iteration
---------

An cursor is a reference to a particular statement in a model.
It can be used to investigate the node IDs in a model,
and, in conjunction with a :struct:`SerdStrings`, the contents of the nodes themselves:

.. literalinclude:: overview_code.c
   :start-after: begin model-strings
   :end-before: end model-strings
   :dedent: 2
...
A more convenient way is to use :func:`serd_model_find_node`.
...
.. literalinclude:: overview_code.c
   :start-after: begin model-find-node
   :end-before: end model-find-node
   :dedent: 2
...
The similar :func:`serd_model_find` instead returns the matching statement:
...
The similar :func:`serd_model_erase_range` will erase all statements in the cursor's range:
...
Models can be copied with :func:`serd_model_copy` and compared with :func:`serd_model_equals`:
...
Destroying a model invalidates all cursors that refer to it.
Node memory is managed by the :struct:`SerdNodes` passed during construction,
and so won't be released until that is destroyed.
