...
and allows redundant string measurement overhead to be avoided.

A :type:`StringView` works in much the same way as ``std::string_view`` (and will likely be replaced with it in the future when C++17 support is more widespread).
For the most part, this simply means you can pass a string literal, other C string, or a ``std::string`` as an argument.
Note, however, that the constructor that takes only a ``const char*`` calls ``strlen`` to measure the string,
so care should be taken in performance-critical code to avoid this if the string length is already known.
...
The datatype or language, if present, can be retrieved with :func:`NodeWrapper::datatype` or :func:`NodeWrapper::language`, respectively.
...
To make a :struct:`Statement` out of this, we need to define some URIs.
...
and can make an equivalent :struct:`Statement`:
...
:func:`StatementWrapper::node`, for example:
...
but if available it can be accessed with :func:`StatementWrapper::cursor`:
...
It is also possible to match statements against a pattern with :func:`StatementWrapper::matches`,
where default-constructed parameters act as a wildcard:
...
and :type:`ModelFlags` which can be used to configure the model:
...
the flags :enumerator:`ModelFlag::index_PSO` and :enumerator:`ModelFlag::store_cursors` could be added like so:
...
This is done by creating a :struct:`ByteSink`,
...
The third argument of :func:`serd_reader_new` takes a bitwise ``OR`` of :enum:`ReaderFlag` flags that can be used to configure the reader.
