...
Data
   A "node" is the basic building block of data,
...
Streams
   ...
   and a :doc:`api/serd_filter` filters statements that match (or don't match) some pattern.

   An event stream describes changes to data and its context,
   but doesn't store the context.
