...
SerdNode*
serd_new_qualified_curie(ZixAllocator* const allocator,
                         const ZixStringView prefix,
                         const ZixStringView suffix)
{
  const size_t    length = prefix.length + 1U + suffix.length;
  SerdNode* const node   = serd_node_malloc(allocator, length, 0, SERD_CURIE);
...
SerdNode*
serd_new_expanded_uri(ZixAllocator* const allocator,
                      const ZixStringView prefix,
                      const ZixStringView suffix)
{
  const size_t    length = prefix.length + suffix.length;
  SerdNode* const node   = serd_node_malloc(allocator, length, 0, SERD_URI);
  char* const     buffer = serd_node_buffer(node);
                      ...
                      const SerdURIView   base)
{
  ...
  SerdNode* const   result  = serd_new_parsed_uri(allocator, abs_uri);
