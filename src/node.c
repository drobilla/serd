...
{
  if (datatype.length && !serd_uri_string_has_scheme(datatype.data)) {
    return result(SERD_ERR_BAD_ARG, 0);
  }
  ...
  return serd_node_construct_literal(
    ...
    serd_substring(temp, r.count),
    SERD_HAS_DATATYPE,
    datatype.length ? datatype : serd_string(NS_XSD "integer"));
}
                           ...
                           const SerdStringView datatype)
{
  ...
  if (!value || !value_size ||
      (datatype.length && !serd_uri_string_has_scheme(datatype.data))) {
    return result(SERD_ERR_BAD_ARG, 0);
  }

  // Determine the type to use (default to xsd:base64Binary)
  const SerdStringView type = datatype.length ? datatype : xsd_base64Binary;
