                             ...
                             const uint8_t     lead)
{
  *size = utf8_num_bytes(lead);
  if (*size < 1) {
    return bad_char(reader, "0x%X is not a UTF-8 leading byte\n", lead);
  }
  ...
  for (uint8_t i = 1U; i < *size; ++i) {
    ...
    if (b == EOF) {
      return r_err(reader, SERD_ERR_BAD_SYNTAX, "unexpected end of input\n");
    }

    const uint8_t byte = (uint8_t)b;
    if (!(byte & 0x80U)) {
      return bad_char(reader, "0x%X is not a UTF-8 continuation byte\n", byte);
    }
