            ...
            SerdCanonicalNode* const out)
{
  ...
  if ((value_type == EXESS_NOTHING) ||
      (r = exess_write_canonical(value_type, str, 0, NULL)).status !=
        EXESS_NO_SPACE) {
    return r;
  }
  ...
  r = exess_write_canonical(value_type, str, buf_size, out->string);
  if (!r.status) {
                        ...
                        SerdCaretView           caret)
{
  ...
  const ExessVariableResult r =
    (object.flags & SERD_HAS_DATATYPE)
      ...
      : build_tagged(object, data->lang_buf, &data->node);
