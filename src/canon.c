      ...
      ...)
{
  ...
  va_start(args, fmt);
  const SerdError e = {
    status, caret.document.string.length ? &caret : NULL, fmt, &args};
                        ...
                        SerdCaretView                 caret)
{
  ...
  if (!(object.flags & (SERD_HAS_DATATYPE | SERD_HAS_LANGUAGE))) {
    return serd_sink_write_event(data->target,
                                 serd_statement_event(flags, statement, caret));
  }
  ...
  const SerdStatus st = serd_sink_write_event(
    ...
    serd_statement_event(
      flags,
      serd_statement_view(
        statement.subject, statement.predicate, node.node, statement.graph),
      caret));
