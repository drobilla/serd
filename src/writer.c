...
SERD_NODISCARD static inline size_t
sink(const void* buf, size_t len, SerdWriter* writer)
{
  const SerdWriteResult r = serd_block_dumper_write(&writer->output, buf, len);
  return r.count;
}
...
write_UCHAR(SerdWriter* const writer, const uint8_t* const utf8)
{
  ...
  SerdWriteResult w = {SERD_SUCCESS, 0U};
  result.read_count = c_size;

  if (result.read_count == 0U) {
    w = wsink(replacement_char, sizeof(replacement_char), writer);
    result.status =
      w_err(writer, SERD_BAD_TEXT, "invalid UTF-8 start: %X", utf8[0]);
  } else if (c <= 0xFFFF) {
    // Write short (4 digit) escape
    snprintf(escape, sizeof(escape), "\\u%04X", c);
    w = wsink(escape, 6, writer);
  } else {
    // Write long (8 digit) escape
    snprintf(escape, sizeof(escape), "\\U%08X", c);
    w = wsink(escape, 10, writer);
  }

  result.status      = w.status ? w.status : result.status;
  result.write_count = w.count;
  return result;
}
...
write_short_text(SerdWriter* writer, const char* utf8, size_t n_bytes)
{
  ...
  for (size_t i = 0; !result.status && i < n_bytes;) {
    ...
    result.status = write_short_string_escape(writer, in);
    if (!result.status) {
      result.read_count = 1U;
    } else if (result.status == SERD_FAILURE) {
    ...
    if (!result.read_count) {
      ...
      result.status = lax ? SERD_SUCCESS : result.status;
    } else {
      i += result.read_count;
    }
  }
...
write_long_text(SerdWriter* writer, const char* utf8, size_t n_bytes)
{
  ...
  for (size_t i = 0; !result.status && i < n_bytes;) {
    ...
    n_quotes        = (utf8[i] == '\"') ? (n_quotes + 1) : 0;

    // Try to write character as a special long escape (newline and friends)
    result.status = write_long_string_escape(writer, n_quotes, last, utf8[i]);
    if (!result.status) {
      result.read_count = 1U;
    } else if (result.status == SERD_FAILURE) {
      // No special escape for this character, write full Unicode escape
