...
nodes_hash(const SerdNode* const node)
{
  ...
  if (serd_node_flags(node) & SERD_HAS_DATATYPE) {
    h = token_hash(h, SERD_URI, serd_node_string_view(meta));
  } else if (serd_node_flags(node) & SERD_HAS_LANGUAGE) {
    h = token_hash(h, SERD_LITERAL, serd_node_string_view(meta));
  }
                                ...
                                const ZixHashInsertPlan plan)
{
  ...
  NodesEntry* const entry = node_entry(node);
