               ...
               const size_t      string_start_offset)
{
  ...
  SerdNode* const temp = push_node(reader, SERD_URI, zix_empty_string());
  if (!temp) {
  ...
  if (!ctx.status) {
    ...
    ctx.status = push_node_termination(reader);
  }
          ...
          SerdNode** const  dest)
{
  ...
  if (!empty) {
    ...
    read_turtle_ws_star(reader);
    *ctx.flags = old_flags;
    st         = serd_sink_write_event(reader->sink,
                               serd_end_event(serd_node_string_view(*dest)));
  }
                  ...
                  bool* const       ate_dot)
{
  ...
  if (st == SERD_FAILURE && (node_has_string(node, true_string) ||
    ...
    node->meta  = push_node(reader, SERD_URI, xsd_boolean);
...
read_turtle_base(SerdReader* const reader, const bool sparql, const bool token)
{
  ...
  TRY(st,
      serd_sink_write_event(reader->sink,
                            serd_base_event(serd_node_string_view(uri))));
                     ...
                     const bool        token)
{
  ...
  st = serd_sink_write_event(
    reader->sink,
    serd_prefix_event(serd_node_string_view(name), serd_node_string_view(uri)));
