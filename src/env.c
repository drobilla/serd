...
serd_env_unset_prefix(SerdEnv* const env, const ZixStringView name)
{
  ...
  const SerdPrefix* const prefix = serd_env_find(env, name.data, name.length);
  if (!prefix) {
    return SERD_SUCCESS;
  }

  // Free prefix strings and clear if it was the only one left
  zix_free(env->allocator, prefix->name.data);
  zix_free(env->allocator, prefix->uri.data);
