...
serd_env_set_base_uri(SerdEnv* const env, const SerdStringView uri)
{
  ...
  if ((env->base_uri_node =
         serd_nodes_get(env->nodes, serd_as_parsed_uri(new_base_uri)))) {
...
serd_env_set_base_path(SerdEnv* const env, const SerdStringView path)
{
  ...
  if (path_last == '/' || path_last == '\\') {
    ...
    base_node = serd_node_new(
      NULL, serd_as_file_uri(serd_string(base_path), serd_empty_string()));

    serd_wfree(env->world, base_path);
  } else {
    base_node = serd_node_new(
      NULL, serd_as_file_uri(serd_string(real_path), serd_empty_string()));
  }
             ...
             const SerdNode* const uri)
{
  ...
  if (prefix) {
    ...
    const SerdNode* const name_node =
      serd_nodes_get(env->nodes, serd_as_string_view(name));
                    ...
                    const SerdStringView uri)
{
  ...
  if (serd_uri_string_has_scheme(uri.buf)) {
    ...
    const SerdNode* const abs_uri =
      serd_nodes_get(env->nodes, serd_as_uri(uri));
    if (!abs_uri) {
      return SERD_BAD_ALLOC;
    }
  ...
  const SerdNode* const abs_uri =
    serd_nodes_get(env->nodes, serd_as_parsed_uri(abs_uri_view));
...
serd_env_expand_node(const SerdEnv* const env, const SerdNode* const node)
{
  ...
  const SerdWriteResult r =
    serd_node_construct(0U, NULL, serd_as_parsed_uri(abs_uri));

  SerdNode* const expanded = serd_node_try_malloc(env->world->allocator, r);
  if (expanded) {
    serd_node_construct(r.count, expanded, serd_as_parsed_uri(abs_uri));
  }
