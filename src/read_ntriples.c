...
read_LANGTAG(SerdReader* const reader, TokenHeader** const dest)
{
  ...
  while (is_alpha((c = peek_byte(reader)))) {
    TRY(st, eat_push_byte(reader, *dest, c));
  }
...
read_EOL(SerdReader* const reader)
{
  ...
  while (is_EOL(peek_byte(reader))) {
    TRY(st, skip_byte(reader, peek_byte(reader)));
  }
...
static SerdStatus
read_IRI_scheme(SerdReader* const reader, TokenHeader* const dest)
{
  SerdStatus st = SERD_SUCCESS;
  int        c  = peek_byte(reader);
  ...
  while (!st) {
    ...
    st = is_scheme(c) ? eat_push_byte(reader, dest, c)
...
*/
static SerdStatus
read_IRI(SerdReader* const reader, TokenHeader** const dest)
{
  SerdStatus st = SERD_SUCCESS;
...
read_PN_CHARS_U(SerdReader* const reader, TokenHeader* const dest)
{
  ...
  return (c == '_') ? eat_push_byte(reader, dest, c)
...
read_PN_CHARS(SerdReader* const reader, TokenHeader* const dest)
{
  ...
  if (is_alpha(c) || is_digit(c) || c == '_' || c == '-') {
    return eat_push_byte(reader, dest, c);
  }
                      ...
                      bool* const         ate_dot)
{
  ...
  st = is_digit(c) ? eat_push_byte(reader, n, c) : read_PN_CHARS_U(reader, n);

  // Read middle: (PN_CHARS | '.')*
  while (!st) {
    c  = peek_byte(reader);
    st = (c == '.') ? eat_push_byte(reader, n, c) : read_PN_CHARS(reader, n);
  }
             ...
             TokenHeader** const meta)
{
  ...
  if (next == '@') {
    TRY(st, skip_byte(reader, '@'));
    TRY(st, read_LANGTAG(reader, meta));
    (*dest)->flags |= SERD_HAS_LANGUAGE;
  } else if (next == '^') {
    TRY(st, skip_byte(reader, '^'));
    TRY(st, eat_byte_check(reader, '^'));
    TRY(st, read_IRI(reader, meta));
