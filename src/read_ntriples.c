...
read_pct_encoded(SerdReader* const reader, SerdNode* const node)
{
  ...
  if (size == 1U && byte != '-' && byte != '.' && byte != '_' && byte != '~' &&
      !is_alpha(byte)) {
    const uint8_t encoded[] = {'%', hex[0], hex[1]};
    return push_bytes(reader, node, encoded, 3U);
  }
                      ...
                      bool* const       ate_dot)
{
  ...
  if (!(*dest =
          push_node(reader,
                    SERD_BLANK,
                    reader->bprefix_len
                      ? zix_substring(reader->bprefix, reader->bprefix_len)
                      : zix_empty_string()))) {
    return SERD_BAD_STACK;
  }
...
read_Var(SerdReader* const reader, SerdNode** const dest)
{
  ...
  if (!(*dest = push_node(reader, SERD_VARIABLE, zix_empty_string()))) {
    return SERD_BAD_STACK;
  }
