...
{
  const SerdNodes* const  nodes = serd_model_nodes(ctx->model);
  const SerdStatementView statement_view =
    serd_statement_statement_view(statement, nodes);

  SerdEvent event = serd_statement_event(flags, statement_view);
  if (ctx->flags & SERD_DESCRIBE_CARET) {
    event.caret = serd_model_statement_caret(ctx->model, statement);
  }
...
{
  return (
    !serd_model_ask_by_id(ctx->model, 0U, ctx->rdf_rest_id, id, 0U) &&
    serd_model_count_by_id(ctx->model, id, ctx->rdf_first_id, 0U, 0U) == 1 &&
    serd_model_count_by_id(ctx->model, id, ctx->rdf_rest_id, 0U, 0U) == 1);
}
...
get_node_style(const DescribeContext* const ctx, const SerdNodeID node_id)
{
  ...
  const SerdTokenView    node  = serd_nodes_get_token(nodes, node_id);

  if (!node_id || node.type != SERD_BLANK) {
    return NAMED; // Non-blank node can't be anonymous
  }

  const size_t n_as_object = serd_model_count_by_id(model, 0U, 0U, node_id, 0U);
  if (n_as_object > 1) {
    return NAMED; // Blank node used as an object several times
  }

  if (ctx->order >= SERD_ORDER_GSPO) {
    const size_t n_as_graph =
      serd_model_count_by_id(model, 0U, 0U, 0U, node_id);
  ...
  if (is_list_head(ctx, node_id)) {
    ...
    if (serd_model_count_by_id(model, node_id, 0U, 0U, 0U) > 2) {
      return LIST_S;
    }
  }

  return n_as_object == 0                                   ? ANON_S
         : serd_model_ask_by_id(model, node_id, 0U, 0U, 0U) ? ANON_O
                                                            : EMPTY_O;
}
           ...
           const SerdNodeID             graph_id)
{
  ...
  const SerdStatement* fs = serd_model_get_statement_by_id(
    model, node_id, ctx->rdf_first_id, 0U, graph_id);
  ...
  while (!st && node_id != ctx->rdf_nil_id) {
    ...
    if ((st = write_range_statement(ctx, depth, flags, fs, 0U, false))) {
      return st;
    }

    // Get rdf:rest statement
    const SerdStatement* const rs = serd_model_get_statement_by_id(
      model, node_id, ctx->rdf_rest_id, 0U, graph_id);
    ...
    fs                       = serd_model_get_statement_by_id(
      model, next_id, ctx->rdf_first_id, 0U, graph_id);
...
write_subject_types(const DescribeContext* const ctx,
                    ...
                    const SerdNodeID             graph_id)
{
  SerdCursor t = serd_model_search_by_id(
                      ...
                      const bool                   write_types)
{
  ...
  if ((st = emit_statement(ctx, statement_flags, statement))) {
    return st;
  }

  if (object_style == ANON_O) {
    // Follow an anonymous object with its description like "[ ... ]"
    SerdCursor iter = serd_model_search_by_id(model, object_id, 0U, 0U, 0U);

    if (!(st = write_range(ctx, depth + 1, &iter, last_subject, false))) {
      st = serd_sink_event(
        sink, serd_end_event(serd_nodes_get_token(nodes, object_id).string));
    }
                    ...
                    const SerdDescribeFlags flags)
{
  ...
  DescribeContext ctx = {
    ...
    sink,
    list_subjects,
    serd_nodes_existing_id(nodes, serd_a_uri(zix_string(NS_RDF "first"))),
    serd_nodes_existing_id(nodes, serd_a_uri(zix_string(NS_RDF "rest"))),
    serd_nodes_existing_id(nodes, serd_a_uri(zix_string(NS_RDF "type"))),
    serd_nodes_existing_id(nodes, serd_a_uri(zix_string(NS_RDF "nil"))),
