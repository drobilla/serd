...
void
serd_reader_set_blank_id(SerdReader* const reader, SerdNode* const node)
{
                 ...
                 const size_t       length)
{
  ...
  *terminator = 0;
  ...
  void* mem = serd_stack_push_aligned(
    &reader->stack, sizeof(SerdNode) + max_length + 1U, sizeof(SerdNode));
                ...
                const SerdSink* const sink)
{
  ...
  me->sink    = sink;
  me->env     = env;
  me->stack   = serd_stack_new(allocator, stack_size, serd_node_align);
  me->syntax  = syntax;
  me->flags   = flags;
  me->next_id = 1;
