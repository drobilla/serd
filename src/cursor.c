...
serd_cursor_get(const SerdCursor* ZIX_NULLABLE cursor)
{
  ...
  if (statement) {
    const SerdStatementView view = {
      // FIXME: Nullable casts
      serd_node_token_view((const SerdNode*)statement->nodes[0]),
      serd_node_token_view((const SerdNode*)statement->nodes[1]),
      serd_node_object_view((const SerdNode*)statement->nodes[2]),
      serd_node_graph_view((const SerdNode*)statement->nodes[3]),
    };

    return view;
  }

  static const SerdTokenView empty_literal = {ZIX_STATIC_STRING(""),
                                              SERD_LITERAL};

  static const SerdStatementView no_statement = {
    empty_literal,
    empty_literal,
    {ZIX_STATIC_STRING(""), SERD_LITERAL, 0U, empty_literal},
    empty_literal};
...
{
  const SerdStatement* const statement = serd_cursor_get_internal(cursor);
  if (!statement || !statement->caret) {
    const SerdCaretView caret = {{{"", 0U}, SERD_LITERAL}, 0U, 0U};
    return caret;
  }

  return serd_caret_caret_view(statement->caret);
}
