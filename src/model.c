...
serd_model_add_index(SerdModel* const model, const SerdStatementOrder order)
{
  ...
  ZixBTree* const index = zix_btree_new(model->allocator, comparator, ordering);
               ...
               const SerdNode* const  g)
{
  ...
  const SerdStatement* const statement =
    serd_model_get_statement_internal(model, s, p, o, g);
...
const SerdStatement*
serd_model_get_statement_internal(const SerdModel* const model,
                                  const SerdNode* const  s,
                                  const SerdNode* const  p,
                                  const SerdNode* const  o,
                                  const SerdNode* const  g)
...
{
  if (!caret.document.string.length) {
    return NULL;
  }
  ...
  if (copy) {
    copy->document =
      serd_nodes_get(model->nodes, serd_a_token_view(caret.document));
...
serd_model_insert_statements(SerdModel* const model, SerdCursor* const range)
{
  ...
  for (const SerdStatement* statement = serd_cursor_get_internal(range);
       !st && statement;
       statement = serd_cursor_get_internal(range)) {
    if (!(st = serd_model_add(model,
                              serd_nodes_intern(nodes, statement->nodes[0]),
                              serd_nodes_intern(nodes, statement->nodes[1]),
                              serd_nodes_intern(nodes, statement->nodes[2]),
                              statement->nodes[3]
                                ? serd_nodes_intern(nodes, statement->nodes[3])
                                : NULL))) {
