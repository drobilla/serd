...
serd_fread_wrapper(void* const buf, const size_t len, void* const stream)
{
...
static SerdStreamResult
serd_file_read_byte(void* const buf, const size_t len, void* const stream)
{
  assert(len == 1U);
  (void)len;
...
static SerdStreamResult
serd_fwrite_wrapper(const void* const buf, const size_t len, void* const stream)
{
  FILE* const file = (FILE*)stream;
